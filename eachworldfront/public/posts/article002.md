...
📋 **plan** 文件夹用于存放每日计划、时间表、待完成事项等内容，帮助高效管理和安排学习、工作任务。可在此文件夹中自由创建每日的计划文档，文档格式可以是 `.txt`、`.md`、`.pdf`、`.xlsx` 等。

📜**Study Materials（学习资料）** 该文件夹用来存放学习资料，里面可以放置一些，例如离散，python，web开发 等资料，不用太局限，心得或者笔记都可以，遇见优质内容也可以分享在其中。Materials_Configuration.md是说明清单，在上传新文件后可以在里面添加一些介绍，后期如果充实起来可以帮助快速了解内容。   

📘 **Study Tracker（学习进度追踪）** 该文件夹用于记录在每个学科的学习进度、感想、反思。您可以通过 Markdown 文件或其他格式（如 `.txt`, `.md`, `.pdf`, `.xlsx`）来整理和更新学习情况。每周追踪进度，让自己清楚掌握的技能有哪些, 用于将来做skill tree。
...
> Each：精简了Readme.md 里的Git的命令，替换为图片形式，具体文件在Study Materials

![GitCheatSheet-ByGeekHour_page-0001](./asset/image/GitCheatSheet-ByGeekHour_page-0001.jpg)
